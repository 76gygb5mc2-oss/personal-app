...
const stats = [
  { label: 'Business Ideas Read', value: 4, max: 10, icon: <TrendingUpIcon />, color: '#ffffff' },
  { label: 'Mindset Principles', value: 2, max: 4, icon: <PsychologyIcon />, color: D.text1 },
...
export default function Reports() {
  return (
    <Container maxWidth="lg" sx={{ py: 4 }}>
      <Box sx={{ mb: 4 }}>
        <Typography variant="h4" fontWeight={800} sx={{ color: D.text1 }}>📊 Reports</Typography>
        <Typography sx={{ color: D.text3, mt: 0.5 }}>Your progress across all modules this month</Typography>
      </Box>

      {/* Overall Score */}
      <Card sx={{ borderRadius: 3, overflow: 'hidden', mb: 3 }}>
        <Box sx={{ background: D.gradHero, p: 4, display: 'flex', alignItems: 'center', gap: 4, flexWrap: 'wrap' }}>
          <Box sx={{ textAlign: 'center' }}>
            <Typography variant="h1" fontWeight={900} sx={{ color: '#ffffff', lineHeight: 1, fontSize: '5rem' }}>72</Typography>
            <Typography sx={{ color: 'rgba(245,240,232,0.6)', fontWeight: 600 }}>Overall Score</Typography>
          </Box>
          <Box sx={{ flex: 1, minWidth: 200 }}>
            <Typography variant="h5" fontWeight={700} sx={{ color: '#ffffff', mb: 0.5 }}>Good Progress 📈</Typography>
            ...
            <Box sx={{ display: 'flex', gap: 1, mt: 2, flexWrap: 'wrap' }}>
              ...
              <Chip label="Best: Fitness" sx={{ bgcolor: 'rgba(245,240,232,0.15)', color: '#ffffff', fontWeight: 600 }} />
            </Box>
          </Box>
        </Box>
      </Card>
      ...
      {/* Module Stats */}
      <Typography variant="h6" fontWeight={700} sx={{ color: D.text1, mb: 2 }}>Module Breakdown</Typography>
          ...
          <Card key={s.label} sx={{ borderRadius: 3 }}>
            <CardContent sx={{ p: 3 }}>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 2 }}>
                ...
                <Typography fontWeight={700} sx={{ color: D.text1, fontSize: '0.9rem' }}>{s.label}</Typography>
              </Box>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                <Typography variant="h4" fontWeight={800} sx={{ color: s.color }}>{s.value}</Typography>
                <Typography sx={{ color: D.text3, alignSelf: 'flex-end', mb: 0.5 }}>/ {s.max}</Typography>
              </Box>
      ...
      {/* Weekly Activity */}
      <Typography variant="h6" fontWeight={700} sx={{ color: D.text1, mb: 2 }}>This Week's Activity</Typography>
            ...
            {weekActivity.map(d => (
              <Box key={d.day} sx={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 1 }}>
                <Box sx={{ width: '100%', borderRadius: 1.5, transition: 'all 0.3s',
                  bgcolor: d.score >= 80 ? '#ffffff' : d.score >= 60 ? 'rgba(255,255,255,0.2)' : '#111111',
                  height: `${(d.score / 100) * 90}px`,
                  '&:hover': { filter: 'brightness(1.1)', transform: 'scaleY(1.05)', cursor: 'pointer' } }} />
                <Typography variant="caption" fontWeight={600} sx={{ color: D.text3 }}>{d.day}</Typography>
              </Box>
            ))}
          </Box>
          <Box sx={{ display: 'flex', gap: 2, mt: 2, flexWrap: 'wrap' }}>
            {[{ label: 'High (80+)', color: '#ffffff' }, { label: 'Medium (60–79)', color: D.text3 }, { label: 'Low (<60)', color: '#111111' }].map(l => (
              <Box key={l.label} sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                <Box sx={{ width: 10, height: 10, borderRadius: 1, bgcolor: l.color }} />
                <Typography variant="caption" sx={{ color: D.text3 }}>{l.label}</Typography>
              </Box>
            ))}
          </Box>
        </CardContent>
